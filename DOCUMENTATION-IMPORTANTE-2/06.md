...
```mermaid
graph TB
    subgraph "migrate dev (Développement)"
        D1[Détecte changements] --> D2[Crée migration SQL]
        D2 --> D3[Applique migration]
        D3 --> D4[Génère client]
        D4 --> D5[Exécute seed]
    end
    
    subgraph "migrate deploy (Production)"
        P1[Lit migrations existantes] --> P2[Compare avec DB]
        P2 --> P3[Applique manquantes]
        P3 --> P4[Ne génère PAS client]
    end
    
    subgraph "db push (Prototypage)"
        B1[Détecte changements] --> B2[Applique directement]
        B2 --> B3[Génère client]
        B3 --> B4[Pas de fichier migration]
    end
